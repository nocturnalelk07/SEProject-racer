...
{
    [BurstCompile]
    public class SquareWaveGenerator : SynthProvider
    {
        [SerializeField, Range(0, 1)] private float amplitude = 0.5f;
        [SerializeField, Range(16.35f, 7902.13f)] private float frequency = 261.62f; // middle C
        [SerializeField] static MusicController musicController;
            ...
            double phase, int sampleRate, float amplitude, float frequency)
        {
            ...
            for (int sample = 0; sample < buffer.Length; sample++)
            {
                ...
                //this is the wave calculation, here we have a square wave
                buffer[sample] = new StereoData(phase < 0.5 ? 1 : -1);
